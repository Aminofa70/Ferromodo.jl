...
plateElem1 = [4,4]
orientation1 = :up
## Warning the origin is not zero

F1,V1 = quadplate(plateDim1,plateElem1; orientation=orientation1)

grid = cogrid(quad4plate, F1,V1)
addnodeset!(grid, "support_1", x -> x[1] ≈ -0.5)  # left side; Dirichlet BC
addnodeset!(grid, "support_2", x -> x[2] ≈ -0.5)  # bottom side; Dirichlet BC
...
display(minimum(u)) ## -0.014285714285714318